...
def draw_bounding_boxes(image, coords, original_size):
    ...
    highlight_color = (190, 195, 0, 128)  # RGBA: Red, Green, Blue, Alpha (128 opacity for a visible highlight)

    # Get the original image size (height, width)
    original_height, original_width = original_size[:2]
    ...
    for box in coords:
        ...
        if len(box) == 4:
            ...
            # Debugging: Print coordinates before scaling
            print(f"Original coords: ymin={ymin}, xmin={xmin}, ymax={ymax}, xmax={xmax}")

            # Scale the coordinates relative to the resized image size
            ...
            # Debugging: Print scaled coordinates for debugging
            print(f"Scaled coords: left={left}, top={top}, right={right}, bottom={bottom}")
        ...
        if st.button('Send to Gemini'):
            if user_input:
                ...
                if results:
                    ...
                    # Check if results are in a list or dict
                    if isinstance(results, list):
                        for item in results:
                            if isinstance(item, list):
                                # Ensure each item in the list is a bounding box like [ymin, xmin, ymax, xmax]
                                if len(item) == 4:  # Ensure the item is a list of 4 coordinates
                                    coords.append(item)
                                else:
                                    print(f"❌ Invalid bounding box format: {item}")
                            elif isinstance(item, dict) and 'box_2d' in item:
                                # If it's a dictionary with 'box_2d', extract the coordinates
                                box_2d = item['box_2d']
                                if len(box_2d) == 4:
                                    coords.append(box_2d)
                                else:
                                    print(f"❌ Invalid bounding box format: {box_2d}")
                    elif isinstance(results, dict):
                        # If it's a single dictionary with "box_2d"
                    ...
                    # If bounding boxes are found, draw them on the image
                    if coords:
                        # Draw bounding boxes and display the image
                        image_with_boxes = draw_bounding_boxes(resized_image, coords, original_size)
