#!/usr/bin/env python3
"""
Faculty-Guided Step Processor v2 - Step-wise Bounding Boxes
...
from ocr.prompt_store import assessment_v23_step_boxes
...
# === Configuration ===
DIM = 1536
BASE_OUTPUT_FOLDER = "/Users/simrannaik/Desktop/subjective_grading/data/stepwise_images/step_wise_box"
...
def merge_coordinates_with_assessment(assessment_json_path, coordinates_data, question_no):
    """Merge coordinates into assessment JSON for specific question"""
    try:
        question_no_float = float(str(question_no).replace('Q', '').replace('q', ''))
        
        print(f"   🔍 Merging coordinates for question {question_no_float}")
        print(f"   📋 Coordinates keys: {list(coordinates_data.keys())}")
        ...
        for item in assessment_data:
            ...
            if metadata.get('question_no') == question_no_float:
                print(f"   ✅ Found matching question in assessment")
                feedback = item.get('feedback', {})
                solution_steps = feedback.get('solution', [])
                print(f"   📊 Number of solution steps: {len(solution_steps)}")
                
                # Add coordinates to each step
                for i, step_data in enumerate(solution_steps):
                    print(f"   🔍 Processing step {i+1}, keys: {list(step_data.keys())}")
                    for key in step_data.keys():
                        if key.startswith('step_') or key.startswith('text_'):
                            step_num = key.split('_')[1]
                            coord_key_full = f"step_{step_num}_coordinates"
                            print(f"      Looking for '{coord_key_full}' in coordinates_data...")
                            
                            # Find matching coordinates (or set to null)
                            if coord_key_full in coordinates_data:
                                step_data["coordinates"] = coordinates_data[coord_key_full]
                                print(f"      ✅ Added coordinates for step {step_num}: {coordinates_data[coord_key_full]}")
                            else:
                                step_data["coordinates"] = None
                                print(f"      ⚠️  '{coord_key_full}' not found, set to null")
                            break
        ...
        print(f"❌ Error merging coordinates: {e}")
        import traceback
        traceback.print_exc()
        return None

# === Dynamic Prompt Template ===
def create_prompt_with_json(student_q_num, faculty_solution, num_student_pages):
    """Create prompt using assessment_v23_step_boxes with JSON faculty solution"""
    ...
    # Create the prompt with faculty solution
    prompt = f"""## Faculty Solution (JSON Format - No Images)
{solution_text}

---

{assessment_v23_step_boxes}"""
...
def draw_bounding_boxes(image, step_boxes):
    """Draw yellow bounding boxes for steps"""
    image = image.convert("RGBA")
    highlight_color = (255, 255, 0, 80)  # Yellow with transparency
    
    print(f"📦 Drawing {len(step_boxes)} step bounding boxes...")
    
    for step_name, step_data in step_boxes.items():
        if not isinstance(step_data, list):
            continue
        
        print(f"   📦 {step_name}: {len(step_data)} boxes")
        
        for i, box_data in enumerate(step_data):
            # Handle both dict format and flat array format
            if isinstance(box_data, dict):
                coords = box_data.get('box_2d', [])
            ...
            overlay_draw.rectangle([left, top, right, bottom], fill=highlight_color, outline=None)
...
def process_single_question(pdf_name, question_number):
    ...
    print(f"✨ Using prompt: assessment_v23_step_boxes from prompt_store")
    
    # Send to Gemini
    print(f"\n📋 Step 6: Sending to Gemini for step-wise boxes...")
    coordinates_data = send_to_gemini_with_json(faculty_solution, resized_student_images, prompt)
    ...
    print(f"📊 Found coordinates for {len(coordinates_data)} steps")
    
    # Save coordinates JSON
    coordinates_json_filename = f"{output_folder}/coordinates.json"
    with open(coordinates_json_filename, 'w') as f:
        json.dump(coordinates_data, f, indent=2)
    print(f"💾 Saved coordinates: {coordinates_json_filename}")
    
    # Merge coordinates with assessment
    if assessment_json_path:
        print(f"\n📋 Step 7: Merging coordinates with assessment...")
        merge_coordinates_with_assessment(assessment_json_path, coordinates_data, question_number)
    
    # Draw step boxes on images
    print(f"\n📋 Step 8: Drawing step boxes on images...")
    
    # Group boxes by page
    boxes_by_page = {}
    for step_name, step_data in coordinates_data.items():
        # Handle array of page-specific coordinates: [{"page": 1, "coords": [...]}, {"page": 2, "coords": [...]}]
        if isinstance(step_data, list) and len(step_data) > 0 and isinstance(step_data[0], dict) and 'page' in step_data[0]:
            for coord_obj in step_data:
                coords = coord_obj.get('coords', [])
                page_num = coord_obj.get('page', 1)
                
                if len(coords) == 4:
                    box = {'box_2d': coords, 'page_number': page_num}
                    if page_num not in boxes_by_page:
                        boxes_by_page[page_num] = {}
                    if step_name not in boxes_by_page[page_num]:
                        boxes_by_page[page_num][step_name] = []
                    boxes_by_page[page_num][step_name].append(box)
                    print(f"   ✅ {step_name}: page {page_num}, coords {coords}")
        
        # Handle single dictionary format: {"page": 1, "coords": [ymin, xmin, ymax, xmax]}
        elif isinstance(step_data, dict) and 'coords' in step_data:
            coords = step_data.get('coords', [])
            page_num = step_data.get('page', 1)
            
            # Flatten nested coords if needed
            if len(coords) == 1 and isinstance(coords[0], list) and len(coords[0]) == 4:
                coords = coords[0]
        ...
        # Handle flat array format: [ymin, xmin, ymax, xmax]
        elif isinstance(step_data, list) and len(step_data) == 4 and all(isinstance(x, (int, float)) for x in step_data):
            box = {'box_2d': step_data, 'page_number': 1}
            page_num = 1
            if page_num not in boxes_by_page:
                boxes_by_page[page_num] = {}
            if step_name not in boxes_by_page[page_num]:
                boxes_by_page[page_num][step_name] = []
            boxes_by_page[page_num][step_name].append(box)
            print(f"   ✅ {step_name}: page {page_num}, coords {step_data}")
        
        # Handle array of objects format: [{"box_2d": [...], "page_number": 1}, ...]
        elif isinstance(step_data, list):
            for box in step_data:
    ...
    for i, resized_student in enumerate(resized_student_images):
        ...
        if page_boxes:
            # Draw combined image with all steps
            combined_image = draw_bounding_boxes(resized_student.copy(), page_boxes)
            combined_filename = f"{output_folder}/{pdf_name}_page{page_num}_combined.png"
            combined_image.convert("RGB").save(combined_filename)
            print(f"💾 Saved combined image (Page {page_num}): {combined_filename}")
            
            # Draw individual step images
            for step_name, step_boxes_list in page_boxes.items():
                step_image = draw_bounding_boxes(resized_student.copy(), {step_name: step_boxes_list})
                step_filename = f"{output_folder}/{pdf_name}_{step_name}_page{page_num}_bbox.png"
                step_image.convert("RGB").save(step_filename)
                print(f"💾 Saved {step_name} image (Page {page_num}): {step_filename}")
...
def process_all_questions():
    ...
    print("🎯 Faculty-Guided Step Processor v2 - Step-wise Bounding Boxes")
    print("=" * 70)
...
if __name__ == "__main__":
    print("🚀 Starting Step-wise Box Processing...")
    print(f"📝 PDF: {PDF_NAME}")
    print(f"✨ Using assessment_v23_step_boxes from prompt_store")
    print()
