...
def draw_bounding_boxes(image, coords, original_size):
    ...
    for box in coords:
        ...
        if len(box) == 4:
            ...
            # Scale the coordinates to the resized image dimensions based on the original image size
    ...
    for box in coords:
        ...
        if len(box) == 4:
            ...
            # Scale the coordinates relative to the original image size and map them to the resized image
            left = int(xmin * image.width)  # Convert to actual pixel position
            top = int(ymin * image.height)  # Convert to actual pixel position
            right = int(xmax * image.width)  # Convert to actual pixel position
            bottom = int(ymax * image.height)  # Convert to actual pixel position
        ...
        if st.button('Send to Gemini'):
            if user_input:
                ...
                if results:
                    ...
                    if coords:
                        # Draw bounding boxes and display the image, passing the original_size for scaling
