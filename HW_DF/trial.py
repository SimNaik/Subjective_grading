...
def pdf_to_images(pdf_path, output_folder):
    ...
    for i in range(num_pages):
        ...
        img_path = os.path.join(output_folder, f"page_{i + 1}.jpeg")
        pix.save(img_path)
...
def main(pdf_file_path, output_folder):
    ...
    for page_num in range(len(images)):
        ...
        # Save resized image
        img_filename_dim = f"DIM_768_PAGE_{page_num + 1}.jpeg"
    ...
    # Step 5: Save the OCR results in a JSON file
    if results:
...
if __name__ == "__main__":
    pdf_file_path = sys.argv[1]  # Get PDF file path from the argument
    output_folder = os.path.dirname(pdf_file_path)  # Use the same folder as the PDF
