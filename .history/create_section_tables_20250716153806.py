...
def copy_content_to_cell(src_doc, cell, images_dir=None):
    """Copy all paragraphs, tables, and images from src_doc into the given cell. Also add images from images_dir if provided."""
    ...
    # Add images from the images_dir if provided
    if images_dir and os.path.exists(images_dir):
        for img_file in sorted(os.listdir(images_dir)):
            img_path = os.path.join(images_dir, img_file)
            try:
                cell.add_paragraph().add_run().add_picture(img_path, width=Inches(2))
...
for section_file in section_files:
    ...
    # Images directory for this section (if exists)
    human_images_dir = os.path.join(human_dir, "images")
    gemini_images_dir = os.path.join(gemini_dir, "images")
    ...
    if human_doc:
        copy_content_to_cell(human_doc, table.cell(1, 0), images_dir=human_images_dir)
    else:
    ...
    if gemini_doc:
        copy_content_to_cell(gemini_doc, table.cell(1, 1), images_dir=gemini_images_dir)
    else:
