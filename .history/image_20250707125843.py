...
# === Prompt for Gemini ===
PROMPT = """
Return bounding boxes as JSON arrays on the line ""
[ymin, xmin, ymax, xmax] 
"""
...
def send_to_gemini(image, prompt):
    try:
        ...
        if response.status_code != 200:
            print(f"❌ Error: {response.status_code}, Message: {response.content}")
            return None
...
def main():
    ...
    prompt_input = st.text_area("Enter the Prompt to Send to Gemini",
        value="Return bounding boxes as JSON arrays on the line "" [ymin, xmin, ymax, xmax]",
        height=200)
    ...
    if uploaded_file is not None:
        ...
        # Button to trigger the LLM process
        if st.button("Send to Gemini"):
            # Send the image and prompt to Gemini for OCR
            st.write("Processing image with Gemini OCR...")
            results = send_to_gemini(resized_image, prompt_input)
