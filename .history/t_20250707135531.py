...
# --- In your Streamlit code, update how you handle the results ---
if results:
    ...
    if isinstance(results, list):
        # If the result is a list of bounding boxes, assume each item is a bounding box
        coords = results
    else:
        # Otherwise, check if it's a dictionary with "box_2d" key
        for item in results:
            if isinstance(item, dict):  # Check if item is a dictionary
                # Check if the dictionary has a "box_2d" key
                box_2d = item.get("box_2d", [])
                if box_2d:
                    coords.append(box_2d)
            elif isinstance(item, list):  # If the item is a list of coordinates
                coords.append(item)  # Directly append the list of coordinates
