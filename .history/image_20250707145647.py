...
def send_to_gemini(image, prompt):
    try:
        ...
        # We assume that the bounding box data is always an array of numbers, e.g. [179, 298, 212, 523]
        if '[' in raw_response and ']' in raw_response:
            # Extract the JSON part (the array between brackets)
            cleaned = raw_response.split("[", 1)[1].split("]", 1)[0]
            cleaned = f"[{cleaned}]"

            # Clean up surrounding backticks if present
            cleaned = cleaned.strip('```json').strip('```').strip()

            # Now try to parse the cleaned response as JSON
            try:
            ...
            print(f"❌ Malformed JSON (no brackets found): {raw_response}")
            return None
