...
def draw_bounding_boxes(image, coords):
    ...
    highlight_color = (190, 195, 0, 134)  # RGBA: Red, Green, Blue, Alpha (opacity: 0-255)
...
def send_to_gemini(image, prompt):
    try:
        ...
        # Clean the raw response by removing any non-JSON content (e.g., text like 'Repulsion:')
        # We assume the bounding box data comes after the first colon and space.
        # Split the string and keep only the part after the first colon.
        if ':' in raw_response:
            cleaned = raw_response.split(":", 1)[1].strip()  # Take everything after the first colon
        else:
            cleaned = raw_response.strip()  # No colon, so use the full response as is

        # Now we should have something that can be parsed as JSON, like [176, 226, 198, 354]
