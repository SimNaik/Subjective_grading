...
def draw_bounding_boxes(img, coords):
    ...
    for box in coords:  # Iterate over bounding boxes
        if isinstance(box, list) and len(box) == 4:
            ymin, xmin, ymax, xmax = box  # Extract the coordinates directly
            
            # Ensure the bounding box is within the image dimensions
            xmin = max(0, min(xmin, image.width))
            ymin = max(0, min(ymin, image.height))
            ...
            overlay_draw.rectangle([xmin, ymin, xmax, ymax], fill=highlight_color)

            # Composite the original image with the overlay (keeping text visible)
            image = Image.alpha_composite(ige, overlay)
