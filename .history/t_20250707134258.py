...
def draw_bounding_boxes(img, coords, original_size, new_size):
    ...
    orig_w, orig_h = original_size[1], original_size[0]  # Extract original image dimensions
    new_w, new_h = new_size  # Extract resized image dimensions
    ...
    for box in coords:  # Assuming coords is a list of bounding boxes
        if isinstance(box, list) or isinstance(box, tuple):  # Ensure box is iterable
            if len(box) == 4:  # Ensure the box has 4 elements (ymin, xmin, ymax, xmax)
                ...
                # Scale the bounding box coordinates based on the resized image
                xmin_scaled = xmin * (new_w / orig_w)
                xmax_scaled = xmax * (new_w / orig_w)
                ymin_scaled = ymin * (new_h / orig_h)
                ymax_scaled = ymax * (new_h / orig_h)

                # Convert to absolute pixel values based on the image size
                left = xmin_scaled  # Directly use scaled xmin
                top = ymin_scaled   # Directly use scaled ymin
                right = xmax_scaled # Directly use scaled xmax
                bottom = ymax_scaled # Directly use scaled ymax
